...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.token_transaction?.length > 0) {
      const create_token_transaction = body.token_transaction;
      body.token_transaction = {
        create: create_token_transaction,
      };
    } else {
      delete body.token_transaction;
    }
