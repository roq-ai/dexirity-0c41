...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getTokenTransactions() {
    const data = await prisma.token_transaction
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'token_transaction'));
  ...
  async function createTokenTransaction() {
    ...
    const data = await prisma.token_transaction.create({
      data: body,
    });
    return res.status(200).json(data);
  }
